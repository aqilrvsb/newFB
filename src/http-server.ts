...
// Helper function to get user's Facebook pages for ad creation
async function getUserFacebookPages(userId: string): Promise<any> {
  try {
    ...
    const response = await fetch(`https://graph.facebook.com/v18.0/me/accounts?fields=id,name,access_token&access_token=${session.credentials.facebookAccessToken}`);
    ...
    return {
      ...
      message: `Found ${pagesData.data?.length || 0} Facebook page(s)`
    };
  } catch (error) {
    return {
      success: false,
      error: `Error fetching pages: ${error instanceof Error ? error.message : 'Unknown error'}`
    };
  }
}
    ...
    switch (toolName) {
      ...
      case 'create_ad_creative':
        try {
          ...
          const name = args.name;
          const pageId = args.pageId || "514433091762412"; // Default to A-Smart Wellness
          const link = args.link || "https://example.com";
          const message = args.message || "Test ad message";

          if (!name) {
            return {
              success: false,
              error: 'name is required for ad creative',
              tool: 'create_ad_creative'
            };
          }

          // Facebook API correct format from documentation
          const params = {
            name: name,
            object_story_spec: {
              page_id: pageId,
